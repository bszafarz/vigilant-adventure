...
currTime = int(time.time())-int(6*24*60*60)
...
for i in range(1, 6):
    if count > 0:
        ...
        if os.path.isfile(currFile):
            ...
            print(f'File {currFile} does not exist - ommiting it')
        count -= 1
    else:
        break
...
if overtime > 0:
    ...
    print(f"You are {abs(int(overtime/60))} min short of work schedule this week!")
else:
